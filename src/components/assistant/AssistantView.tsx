    ...
    <div className={`flex items-start gap-2.5 ${isUser ? 'flex-row-reverse' : ''}`}>
      ...
      <div className="flex flex-col gap-1.5 max-w-[78%]">
        ...
        {!isUser && msg.sources && msg.sources.length > 0 && (
          <SourceList sources={msg.sources} ragFilter={msg.ragFilter} />
        )}
      </div>
    </div>
  )
...
function SourceList({ sources, ragFilter }: { sources: SourceCitation[]; ragFilter?: RagFilter }) {
  const [expanded, setExpanded] = useState(false)
  const pluginSources = sources.filter((s) => s.isPlugin)
  const pluginCount = pluginSources.length
  const uniquePlugins = new Set(pluginSources.map((s) => s.pluginSlug).filter(Boolean))
 ...
 */
function RetrievalScopeBar({ filter, pluginCount }: { filter: RagFilter; pluginCount: number }) {
...
/**
 * Source citation card. Plugin sources render a prominent
 * [Plugin: <slug>] badge so the user can see provenance at a glance.
 */
...
function prettyPluginName(slug: string | null): string {
  ...
  return slug
    ...
    .map((w) => {
      ...
      // Short tokens like "1", "16", "ii" → keep as-is or roman numeral
